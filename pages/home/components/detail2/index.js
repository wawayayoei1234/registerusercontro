...
function index() {
  ...
  return (
    ...
    {homedata.map((data,index)=>
        <div key={`${index}`}>
        <Container sx={{ display: { xs: 'none', md: 'flex' }, my:"60px"}}  maxWidth="false">
        <Box sx={{mr:10}}></Box>
        <Box>
            <Box sx={{color:"secondary.main",fontFamily:"Segoe UI",fontWeight:"300",fontSize:"20px",'@media (max-width: 600px)': {fontSize: '10px'}}} className="detail1">
              {language==="EN"&&(data.detail2_1EN)}
              {language==="TH"&&(data.detail2_1TH)}
          </Box>
            <Box sx={{color:"secondary.main",fontFamily:"Segoe UI",fontWeight:"300",fontSize:"20px",'@media (max-width: 600px)': {fontSize: '10px'}}} className="detail2">
              {language==="EN"&&(data.detail2_2EN)}
              {language==="TH"&&(data.detail2_2TH)}
          </Box>
        </Box>
        </Container>

        <Container sx={{ display: { xs: 'flex', md: 'none' }, my:"60px",justifyContent:"center",alignItems:"center",flexDirection:"column"}}  maxWidth="false">
